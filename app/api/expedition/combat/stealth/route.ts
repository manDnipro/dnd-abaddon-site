...
export async function POST() {
  ...
  // Sneak attack only makes sense as the opening move, before the enemy already knows you're there.
  if (combat.hp !== combat.maxHp) return NextResponse.json({ error: 'Момент несподіванки вже втрачено' }, { status: 400 })

  let log: string[] = []
  const playerRound = resolvePlayerAttack(character, combat, true)
