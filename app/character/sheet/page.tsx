...
import { Character, STAT_LABELS, StatKey, statModifier, formatModifier } from '@/lib/types'
...
export default function CharacterSheetPage() {
  ...
  return (
    <div>
      ...
      <div className="card mb-4">
        <h2 style={{ color: '#c9a227', fontSize: 14, marginBottom: 10 }}>Досягнення</h2>
        <div className="flex flex-col gap-2" style={{ fontSize: 13 }}>
          <div className="flex justify-between"><span style={{ color: '#888' }}>🎯 Вилазок завершено</span><span style={{ color: '#c9a94f' }}>{character.expeditionsCompleted}</span></div>
          <div className="flex justify-between"><span style={{ color: '#888' }}>💀 Зомбі знищено</span><span style={{ color: '#c9a94f' }}>{character.zombiesKilled}</span></div>
          <div className="flex justify-between"><span style={{ color: '#888' }}>🩹 Гравців врятовано</span><span style={{ color: '#c9a94f' }}>{character.playersSaved}</span></div>
        </div>
      </div>
